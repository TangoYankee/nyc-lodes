...
```
Create template map layer of brooklyn subway lines
```{r, message = FALSE}
...
tmap_arrange(bk_intra_nta_od_dist_02, bk_intra_nta_od_dist_19)
```
...
bk_desire_lines_dist_02 <- tm_shape(bk_nta_borders) +
  tm_polygons(
    col = "#e4e4e4",
  ) +
...
bk_desire_lines_dist_19 <- tm_shape(bk_nta_borders) +
  tm_polygons(
    col = "#e4e4e4",
  ) +
  bk_mono_subway_lines_map +
...
tmap_arrange(bk_desire_lines_dist_02, bk_desire_lines_dist_19)
```
